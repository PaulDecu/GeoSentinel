...
export default function GeolocationScreen({ navigation }) {
  ...
  useEffect(() => {
    console.log('🔄 État changé - isTracking:', isTracking, 'tourneeType:', tourneeType);
  ...
  const loadTourneeType = async () => {
    try {
      ...
      console.log('🔍 loadTourneeType - Type:', typeof savedType);
      
      if (savedType) {
        setTourneeType(savedType);
        console.log('✅ Type de tournée chargé:', savedType);
      } else {
        console.log('⚠️ Aucun type de tournée sauvegardé');
      }
    } catch (error) {
  ...
  return (
    <ScrollView style={styles.container} key={refreshKey}>
      <View style={styles.content}>
        ...
        {/* Affichage du type actif - VERSION DEBUG */}
        {(() => {
          console.log('🎨 RENDER - isTracking:', isTracking, 'tourneeType:', tourneeType);
          return null;
        })()}
        
        {/* VERSION 1: Affichage conditionnel normal */}
        {isTracking && tourneeType && (
          <View style={styles.infoCard}>
        ...
        {/* VERSION 2: DEBUG - Affichage FORCÉ pour voir les états */}
        {isTracking && (
          <View style={[styles.infoCard, { backgroundColor: '#FFE4E1' }]}>
            <Text style={styles.infoTitle}>🔍 DEBUG - États</Text>
            <View style={styles.activeTourneeBox}>
              <Text style={styles.activeTourneeText}>
                isTracking: {isTracking ? '✅ OUI' : '❌ NON'}
              </Text>
              <Text style={styles.activeTourneeText}>
                tourneeType: "{tourneeType || 'VIDE'}"
              </Text>
              <Text style={styles.activeTourneeText}>
                Label: {getTourneeLabel(tourneeType) || 'AUCUN'}
              </Text>
            </View>
          </View>
        )}
